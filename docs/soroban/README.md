...
## ⚠️ Status: scaffold only — not the live contracts

The crates in `soroban/contracts/` (`escrow`, `program-escrow`) are the
unmodified hello-world placeholders generated by `stellar contract init`
during the initial workspace setup (step 3 below). They contain **no escrow
logic**, are **not deployed**, and have not changed since the initial commit.

The live, actively maintained implementations live elsewhere in the repo:

| Scaffold crate (this workspace) | Live implementation |
| --- | --- |
| `soroban/contracts/escrow` | `bounty_escrow/contracts/escrow/src/lib.rs` |
| `soroban/contracts/program-escrow` | `program-escrow/src/lib.rs` |

**All bug fixes and security patches (reentrancy, overflow, fund-safety,
etc.) must be applied to the live implementations only.** This relationship
was audited and documented in issue #195.
